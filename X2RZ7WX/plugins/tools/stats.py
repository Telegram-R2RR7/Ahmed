...
from X2RZ7WX.core.userbot import assistants
from X2RZ7WX.misc import SUDOERS, pymongodb
from X2RZ7WX.plugins import ALL_MODULES
from X2RZ7WX.utils.database import (get_global_tops,
                                       get_particulars, get_queries,
                                       get_served_chats,
                                       get_served_users, get_sudoers,
                                       get_top_chats, get_topp_users)
from X2RZ7WX.utils.decorators.language import language, languageCB
from X2RZ7WX.utils.inline.stats import (back_stats_buttons,
                                           back_stats_markup,
                                           get_stats_markup,
...
async def gstats_global(client, message: Message, _):
    ...
    title = title.title()
    final = f"اكثر اغنية تم تشغيلها بالكرة\n\n**العنوان :** {title}\n\nتم تشغيلها** {co}0 **مرة"
...
@app.on_message(filters.command("احصائيات الكرة العالمية", [".", ""]) & filters.group)
@languageCB
async def gstats_global(client, message: Message, _):
    mystic = await message.reply_text(_["gstats_1"])
    stats = await get_global_tops()
    ...
    def get_stats():
        ...
        if what in ["Tracks", "Here"]:
            for items, count in list_arranged.items():
                ...
                if items == "telegram":
                    msg += f"- [مقاطع صوتية](https://t.me/) ** تم تشغيلها {count}0 مرة**\n\n"
                else:
